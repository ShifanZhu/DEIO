...
def train(rank, args):
    ...
    eVONet(args, **kwargs_net, patch_selector=args.patch_selector.lower(), norm=args.norm, randaug=args.randaug)
    ...
    with torch.profiler.profile(
        ...
        while True:
            ...
            for data_blob in train_loader:
                ...
                loss = 0.0
                pose_loss = 0.0 #所谓的pose loss就是相对位置误差
                flow_loss = 0.0 #所谓的flow loss就是重投影误差（需要给定GT depth）
                ...
                for i, data in enumerate(traj):
                    ...
                    loss += args.flow_weight * flow_loss # flow loss及其对应的权重
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain', type=str, default="resnet34", help='pretrained ResNet model (resnet18, resnet34)')
    
    args = parser.parse_known_args()[0]

    # 解析 block_dims 字符串为列表
    block_dims = list(map(int, args.block_dims.split(',')))
    # 将 block_dims 存储到 args 中
    args.block_dims = block_dims

    assert_config(args)# check config 是否合适
